...
function buscarAlumno() {
    ...
    fetch(`http://localhost:8104/api/alumnos/documento/${numeroDocumento}`)
        .then(response => {
            if (!response.ok) {
        ...
        .then(alumno => {
            ...
            div.innerHTML = `
                ...
                <p><strong>Teléfono:</strong> ${alumno.numeroTelefonico || ''}</p>
                <p><strong>Formulación Proyectos Ing:</strong> ${alumno.formulacionDeProyectosDeIngenieria || ''}</p>
                <p><strong>Nivel Formulación Proyectos Ing:</strong> ${alumno.formulacionDeProyectosDeIngenieriaNivel || ''}</p>
                <p><strong>Pensamiento Científico:</strong> ${alumno.pensamientoCientificoMatematicasYEstadistica || ''}</p>
                <p><strong>Nivel Pensamiento Científico:</strong> ${alumno.pensamientoCientificoMatematicasYEstadisticaNivel || ''}</p>
                <p><strong>Diseño SW:</strong> ${alumno.disenoDeSoftware || ''}</p>
                <p><strong>Nivel Diseño SW:</strong> ${alumno.disenoDeSoftwareNivel || ''}</p>
                <p><strong>Nivel Inglés:</strong> ${alumno.nivelIngles || ''}</p>
            `;
        })
        .catch(error => {
...
function editarAlumno(id) {
    fetch(`http://localhost:8104/api/alumnos/${id}`)
        .then(res => res.json())
        .then(alumno => {
